...
export const GET: RequestHandler = async (event) => {
  try {
    ...
    return json(await listRequestWorkspaceSessions(event, { archived }), { headers: createWorkspaceRuntimeDiagnosticHeaders(event) });
  } catch (caught) {
...
function createWorkspaceRouteErrorBody(caught: unknown): { ok: false; error: string; code?: string } {
  ...
  return { ok: false, error: "Workspace sessions request failed" };
}
